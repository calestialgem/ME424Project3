...
function N_t_min = find_yield_limit(self)
	K_s = 1+0.5/self.C;
	S_ut = material.find_strength(self.d);
	S_ys = S_ut*given.S_ys_percentage;
...
function print(self)
	D = self.C*self.d;
	N_a = self.N_t-2;
	k = given.G*self.d/8/self.C^3/N_a;
	...
	S_ut = material.find_strength(self.d);
	S_us = S_ut*given.S_us_percentage;
	S_y = S_ut*given.S_y_percentage;
	S_ys = S_ut*given.S_ys_percentage;
	tau_s = K_s*8*F_s*self.C/pi/self.d^2;
	SF_yield = S_ys/tau_s;
	S_s = S_ut*given.S_s_percentage;
